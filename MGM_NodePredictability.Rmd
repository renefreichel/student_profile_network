...
set.seed(8)
sampleidx <- sample((seq_len(nrow(dataMGM))), size = floor(.8 * nrow(dataMGM)))
```

```{r}
# Fitting a MGM with the mgm-package
set.seed(1)
fit_obj <- mgm(data = na.omit(dataMGM[sampleidx,]), # na.omit excludes N = 32 
                type = c(rep("g", 5),"c"),
                level = c(rep(1, 5), 2),
                ruleReg = "OR",
                k = 2,
...
```{r}
# Node Predictability estimation
p_obj <- predict(fit_obj, na.omit(dataMGM[-sampleidx,]),
                  errorCat = c("CC","nCC","CCmarg"),
                  errorCon = c("R2"))
...
# Fitting a MGM with the mgm-package
set.seed(1)
fit_obj <- mgm(data = (dataMGM1719[,-6]), # na.omit excludes N = 32 
                type = c(rep("g", 5)),
                level = c(rep(1, 5)),
                ruleReg = "OR",
                k = 2,
